...
//  CSCI 3800
//  Lab 10
//  Created by Christian Mundwiler on 5/1/22.
/*
    ...
    the program prints 'NOT IN GRID'. This program incorporates the concept of "hop count". When a message is sent, it has a hop count of 4.
    ...
    its port number onto the path. This program incorporates functionality to handle MOV commands; these commands include a location to move to.
    When a node receives this command, they move to the new location and update their coordinates accordingly. This program allows for the
    ...
    before it is deleted. This program incorporates functionality to handle LOC commands; this command requests a nodes location, and any node in
    the network can receive the command from the sender regardless of distance. This program allows for msgs to be displayed to the user in the order
    that they are sent.
            ...
            if(sscanf(networkBuffer, "%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%100[^\n]", versionNumFrom, msgTypeFrom, locationFrom, portFrom, portTo, hopFrom, msgIDFrom, path, msgFrom) == 9) {
                ...
                if (strcmp(msgTypeFrom, msgType3) == 0) {
                    ...
                    if (strtol(portTo, NULL, 10) == myPortNumInt) {
                        ...
                        getCoord(numRows, numCols, &myRow, &myCol, myLocationInt);
                        //printf("MOV command. My new location: %d\n", myLocationInt);
                        //printf("\nEnter: <destination port> & <message>:\n");
                ...
                // check if LOC msg
                if (strcmp(msgTypeFrom, msgType4) == 0) {
                    
                    // check if for me
                    if (strtol(portTo, NULL, 10) == myPortNumInt) {
                        // set msgID
                        msgID = myNodes.nodeInfo[29].seqSent;
                        myNodes.nodeInfo[29].seqSent++;
                        // create message struct
                        message *newLoc = NULL;
                        char locChar[3] = "";
                        char locProt[100];
                        sprintf(locChar, "%d", myLocationInt);
                        // populate struct
                        newLoc = fillMessage(VERSION, msgType4, myLocationInt, myPortNumInt, 2030, 4, msgID, myPortNum, locChar);
                        // make protocol string
                        concat(locProt, newLoc);
                        
                        // set socket parameters and send msg
                        node_addr.sin_family = AF_INET;
                        node_addr.sin_port = htons(2030);
                        node_addr.sin_addr.s_addr = inet_addr(ipAddr);
                        sendString(sockD, locProt, node_addr);
                        // flush buffers
                        memset(locProt, 0, 100);
                        memset(locChar, 0, 3);
                    }
                }
                else {
                    ...
                    if (distance <= 2 && hop >= 0) {
                        ...
                        if (strtol(portTo, NULL, 10) == myPortNumInt) {
                            ...
                            if (strcmp(msgTypeFrom, msgType1) == 0) {
                                // check if msg in order
                                if (msgIDFromInt == idCount) {
                                    // variables for ACK
                                    int pFrom, pTo;
                        ...
                        // if msg not for me, forward
                            ...
                            if(checkPath(path, myPortNumInt)) {
                                duplicate = true;
                            }
...
// func to multicast messages to neighbors
